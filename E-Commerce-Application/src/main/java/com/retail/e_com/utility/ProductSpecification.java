...
public class ProductSpecification {
	public Specification<Product> buildSpecification(SearchFilter filter){
		...
		return (root,query,builder)->{
			...
			if(filter.getAvailabilityStatus()!=null)
				predicate.add(builder.equal(root.get("availabilityStatus"), ProductAvailabilityStatus.valueOf(filter.getAvailabilityStatus())));
